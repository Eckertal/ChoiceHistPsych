...
patsy_a <- 'response ~ (1|sbj_id) + (1|block:sbj_id) + target_z*evidence_z + resp_1_z*evi.1_z*PPS_z'

patsy_a_full <- 'response ~ (1|sbj_id) + (1|block:sbj_id) + target_z*evidence_z + stim_1_z*evi.1_z + resp_1_z*PPS_z*block_type_z + cue_z*PPS_z*block_type_z + resp_1_z*evi.1_z*PPS_z'
...
model1_a_full <- glmer(patsy_a_full, data=data_aud, 
                       family=binomial('logit'), 
                       control=glmerControl(optimizer='optimx', optCtrl=list(method='nlminb')))

# save model in summary text file for later manip. in python
sink('new_exp1_prevDiff.csv')
print(summary(model1_a))
sink()

sink('new_exp1_full_plus_prevDiff.csv')
print(summary(model1_a_full))
sink()
...
patsy_v <- 'response ~ (1|sbj_id) + (1|Block:sbj_id) + target_z*coherence_z + stim_1_z*prev_coh_z + resp_1_z*prev_coh_z*PPS_z '

patsy_v_full <- 'response ~ (1|sbj_id) + (1|Block:sbj_id) + target_z*coherence_z + stim_1_z*prev_coh_z + resp_1_z*PPS_z*block_type_z + cue_z*PPS_z*block_type_z + resp_1_z*prev_coh_z*PPS_z'
...
model1_v_full <- glmer(patsy_v_full, data=data_vis,
                       family=binomial('logit'),
                       control=glmerControl(optimizer='optimx', optCtrl = list(method='nlminb')))


# save model summary in text file
sink('new_exp2_prevDiff.csv')
print(summary(model1_v))
sink()

sink('new_exp2_full_plus_prevDiff.csv')
print(summary(model1_v_full))
sink()
